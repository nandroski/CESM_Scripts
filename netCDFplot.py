...
def plot_contour_fromFile(input_file, parameter, horz_ax: Literal['lon', 'lat', 'lev'], vert_ax: Literal['lon', 'lat', 'lev'],
                 cmap='cubehelix', title=None, levels=10, output_file=None,
                 show=True, save=False, flip_y=False, plot_size=(12, 8),
                 axlabel_size=14, titlelabel_size=16, aspect_ratio="2:1",
                 cbar_ticks=None, y_ticks=None,
    ...
    y_ticks (default = None) : An array containing the tick number labels to be used for the y-axis.
...
def plot_contour(parameter, horz_ax: Literal['lon', 'lat', 'lev'], vert_ax: Literal['lon', 'lat', 'lev'],
                 cmap='cubehelix', title=None, levels=10,
                 output_file=None, show=True, save=False, flip_y=False,
                 plot_size=(12, 8), axlabel_size=14, titlelabel_size=16,
                 aspect_ratio="2:1", cbar_ticks=None, y_ticks=None,
                 extend=False, center=False, log_y=False):
    '''
    Plots a 2D contourf plot for a given parameter along two axes.
    ...
    mean_value = np.mean(np.abs(parameter)) # mean of absolute value
    order_of_mag = np.floor(np.log10(np.abs(mean_value))).values
    print(order_of_mag)
    print(f'Max Value: {max_value:.2E}')
    print(f'Min Value: {min_value:.2E}')
    ...
    X,Y = np.meshgrid(parameter.coords[horz_ax].values,parameter.coords[vert_ax].values)
    ...
    #plot contour
    if center and extend:
        CF = ax.contourf(X,Y,parameter,levels = levels,cmap=cmap,
                         norm=Midpoint_Normalize(midpoint=0,vmin=min_level,vmax=max_level),
                         extend='both')
    elif center:
        CF = ax.contourf(X,Y,parameter,levels = levels,cmap=cmap,
        ...
        CF = ax.contourf(X,Y,parameter,levels = levels,cmap=cmap,extend='both')        
    else:
        CF = ax.contourf(X,Y,parameter,levels = levels,cmap=cmap)
